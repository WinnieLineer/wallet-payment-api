...
class TransactionService(private val walletService: WalletService) {
    ...
    fun topUp(request: TopUpRequest): TransactionResponse = transaction {
        ...
        val wallet = walletService.getWallet(request.walletId)
    ...
    fun payment(request: PaymentRequest): TransactionResponse = transaction {
    ...
    fun transfer(request: TransferRequest): TransactionResponse = transaction {
        // Check for duplicate transaction
        val existingTransaction = findTransactionByIdempotencyKey(request.idempotencyKey)
        if (existingTransaction != null) {
            return@transaction TransactionResponse(
        ...
        // Validate wallets
        val fromWallet = walletService.getWallet(request.fromWalletId)
            ?: throw IllegalArgumentException("Source wallet not found")
        val toWallet = walletService.getWallet(request.toWalletId)
        ...
        val outTransactionId = Transactions.insertAndGetId {
        ...
        val inTransactionId = Transactions.insertAndGetId {
    ...
    fun getTransactionsByWallet(walletId: UUID, fromDate: LocalDate?, toDate: LocalDate?): List<Transaction> = transaction {
        var query = Transactions.select { Transactions.walletId eq walletId }
        
        fromDate?.let { 
            query = query.andWhere { Transactions.createdAt greaterEq it.atStartOfDay().toInstant(java.time.ZoneOffset.UTC) }
        }
        toDate?.let { 
            query = query.andWhere { Transactions.createdAt lessEq it.plusDays(1).atStartOfDay().toInstant(java.time.ZoneOffset.UTC) }
        }
    ...
    fun getTransactionStatus(transactionId: UUID): Transaction? = transaction {
    ...
    private fun findTransactionByIdempotencyKey(key: String): Transaction? = transaction {
    ...
    private fun updateTransactionStatus(transactionId: UUID, status: TransactionStatus) {
