...
class ReconciliationService() {
    ...
    fun reconcileTransactions(
        date: LocalDate,
        externalTransactions: List<TransactionRecord>,
    ...
    fun generateMockExternalFile(date: LocalDate): List<TransactionRecord> =
        transaction {
            val dbReport = Json.decodeFromString<DailyTransactionReport>(generateDailyReport(date))
    ...
    private fun convertToCsv(report: DailyTransactionReport): String {
        ...
        val rows =
            report.transactions.joinToString("\n") { tx ->
                "${tx.transactionId},${tx.walletId},${tx.type},${tx.amount},${tx.currency}," +
                    "${tx.status},${tx.referenceId ?: ""},${tx.timestamp}"
            }
        return header + rows
    }

    private fun areTransactionsEqual(
    ...
    private fun findDiscrepancies(
        id: String,
        dbTx: TransactionRecord,
        extTx: TransactionRecord,
